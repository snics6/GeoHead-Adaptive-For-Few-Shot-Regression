...
= \underbrace{(\Sigma_s^{-1} - \Sigma_t^{-1}) c_s}_{A:\ \text{geometry shift}}
+ \underbrace{\Sigma_t^{-1}(c_s - c_t)}_{B:\ \text{coupling shift}}.
$$
これを risk に代入:
$$
...
- $L_{\text{src}}(B_i)$ は corpus $i$ での MSE（現 episode における "source"）。
  最終的には **全 corpus の MSE を平均** して supervised signal としても良い（sweep 対象）。
...
$$
k_d = \min\Bigl\{ k :\ \frac{\sum_{i=1}^k \lambda_{d,i}}{\sum_{i=1}^p \lambda_{d,i}} > T \Bigr\}.
$$
...
pseudo-inverse:
$$
G_d^+ = V_d\, \mathrm{diag}\Bigl(\tfrac{1}{\lambda_{d,1}},\ldots,\tfrac{1}{\lambda_{d,k}}, 0,\ldots,0\Bigr) V_d^\top.
$$
...
選ばれた `k` 個の主固有値について:
$$
L_{\text{scale}} = \bigl\|\lambda_{s,1:k} - \lambda_{t,1:k}\bigr\|_2.
$$
...
- **Test-time adaptation** on test corpus $T_k$: support $S_t \subset T_k$ で
  $$
  \hat\beta = \arg\min_\beta \sum_{(x,y)\in S_t}(y-\beta^\top \phi_\theta(x))^2 + \lambda \|\beta - \beta_0\|^2
  $$
  closed-form ridge 解で求める。

### 8.2 Baseline 2: `DARE+geo`

- **Train**: Baseline 1 と完全に同じ。
- **Test-time adaptation のみ違う**: geometry-aware head reg
  $$
  ...
  $$
  $\hat\Sigma_t$ は test corpus $T_k$ の support + unlabeled part から推定。
  closed-form 解。
...
│   │   └── head.py          # 線形 head
│   ├── losses/
│   │   ├── dare_gram.py     # 節 5 の exact 実装
│   │   └── head_reg.py      # geometry-aware reg
│   ├── adaptation/
│   │   ├── ridge.py         # closed-form / GD ridge
│   │   └── inner_loop.py    # MAML inner (with `higher`)
│   ├── training/
│   │   ├── baseline.py      # DARE のみの train loop
│   │   └── geohead.py       # bilevel train loop
│   ├── evaluation/
│   │   ├── metrics.py       # MSE/MAE, head correction size
│   │   └── visualize.py     # Gram spectra, cos sim
│   └── utils/
│       ├── seed.py
│       └── config.py
├── experiments/
...
│   │   ├── run_baseline.py
│   │   ├── run_geohead.py
│   │   └── run_all.sh       # 全 method × target × k を回す
│   └── results/             # gitignore
└── tests/
    ├── test_dare_gram.py    # k選択, cos, scale, OLS 整合
    ├── test_inner_loop.py   # gradient flow 確認
    └── test_toy_data.py     # shift が意図通り入っているか
```
...
### M1: 基盤（実装 1〜2 日）
- [ ] toy data generator (`src/geohead/data/toy.py`)
- [ ] encoder + head
- [ ] DARE-GRAM loss の **単体テスト**（OLS 解との整合確認）
- [ ] geometry-aware head reg
- [ ] baseline 1 (`DARE+ridge`) の train→evaluate が走る

### M2: ベースライン整備
- [ ] baseline 2 (`DARE+geo`) を test-time adaptation 含めて完成
- [ ] 評価メトリクスと可視化を一通り出す

### M3: 提案手法
- [ ] inner loop を `higher` で実装
- [ ] bilevel train loop
- [ ] proposed の sanity check（toy で baseline を上回るか）

### M4: 実験本番
- [ ] 4 target conditions × 5 support sizes × 5 seeds
- [ ] sample efficiency curve
- [ ] head correction size の集計

### M5: 分析と ablation
- [ ] subspace 可視化
- [ ] ablation （`w/o DARE`, `w/o headreg`, `first-order`）
